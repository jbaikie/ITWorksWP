					...
					<li>
						...
						<ul class="company-social">
							...
							<?php if ( class_exists( 'WP_Job_Manager_Cat_Colors' ) ) : ?>
								<li><a href="<?php echo get_term_link( $category, 'job_listing_category' ); ?>" class="job-category <?php echo get_the_job_category() ? sanitize_title( get_the_job_category()->slug ) : ''; ?>"><?php the_job_category(); ?></a></li>
							<?php else : ?>
								<li><a href="<?php echo get_term_link( $category, 'job_listing_category' ); ?>"><i class="icon-tag"></i> <?php echo $category->name; ?></a></li>
							<?php endif; ?>
