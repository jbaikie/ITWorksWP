...
function optinrev_js()
{
  ...
  wp_enqueue_script( 'optinrevjsv', OPTINREV_DIR . 'js/optinrev-js.php?page='.urlencode($plugin_page).'&t=' . optinrev_cid(), array(), $wp_version, true );
...
function optinrev_wphead() {
  ...
  $js = sprintf('<link rel="stylesheet" href="'. $dir .'optinrev-css.php?view=%s&t=%s" type="text/css" media="all" />', $optin_id, optinrev_cid());
  $js .= sprintf('<script type="text/javascript" src="'. $dir .'js/optinrev-popup-vars.php?popup=%s&t=%s"></script>', urlencode($optin_id), optinrev_cid());
